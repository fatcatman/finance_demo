...
var model = avalon.define({
    ...
    imageId: 0,
    params:'',
    CurrentObj: { id: '', content: ''},
    isAdd:true,
    load: function () {
        ajax.welcome.List(model.currentPage, model.limit, model.params).done(function (data) {
    ...
    ShowUpdatePic: function (obj) {
        ...
        model.CurrentObj.content = obj.content;
        model.isAdd = false;
        $('#Add_modal').modal('show');
    },
    Save: function () {
        if ($.trim(model.CurrentObj.content) == '') {
            rootVm.$alert('提示', '欢迎语不能为空', 'error');
            return;
        }
        ajax.welcome.save(model.CurrentObj.$model).done(function (data) {
            if (data.success) {
    ...
    Delete: function () {
        ajax.welcome.del(model.CurrentObj.ID).done(function (data) {
            if (data.success) {
