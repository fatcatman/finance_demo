...
var model = avalon.define({
    ...
    Save: function () {
        if ($.trim(model.currentManager.Account) == '') {
            rootVm.$alert('提示', '账号不能为空', 'error');
            return;
        }
        if ($.trim(model.currentManager.Pwd) == '') {
    ...
    ShowDelete : function(obj){
        model.currentManager.ID = obj.ID;
    ...
    ShowUpdatePwd: function (obj) {
        model.currentManager.ID = obj.$model.ID;
        model.currentManager.Account = obj.$model.Account;
        model.currentManager.Pwd = obj.$model.Pwd;
        model.currentManager.role =obj.$model.role.ID;
