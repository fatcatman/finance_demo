...
public class AuthorityDaoImpl implements AuthorityDao {
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<AcctAuthority> List(int page,int size) {
		List<AcctAuthority> query = this.getCurrentSession().createQuery("from AcctAuthority").setFirstResult(page-1).setMaxResults(size).list();
		return query;
	}
