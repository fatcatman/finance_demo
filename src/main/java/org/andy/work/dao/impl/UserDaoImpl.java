...
public class UserDaoImpl implements UserDao {
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<AcctUser> List(int page,int size) {
		List<AcctUser> query = this.getCurrentSession().createQuery("from AcctUser").setFirstResult(page-1).setMaxResults(size).list();
		return query;
	}
